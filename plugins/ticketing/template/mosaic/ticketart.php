...
$qapedit = BASE_URL . 'admin/index.php?p=ticketing&amp;sp=quickedit&amp;id=' . $PAGE_ID; ?>
  ...
  <div id="printdiv">
    <h2
      class="first-child text-color hidden-xs"><?php if (isset($JAK_ST_OPT) && is_array($JAK_ST_OPT)) foreach ($JAK_ST_OPT as $sto) {
        if ($sto["id"] == $row['typeticket'] && $sto["img"]) { ?><a href="<?php echo $sto["parseurl"]; ?>"
                                                                    title="<?php echo $sto["name"]; ?>"><i
            class="fa <?php echo $sto["img"]; ?>"></i></a>
        <?php }
      } ?> <?php echo $TICKET_TITLE; ?></h2>
    ...
    <strong><?php echo $tlt["st"]["t35"]; ?>:</strong> <span
      ...
      :</strong> <span class="label label-default"><?php echo $row["toption"]; ?></span>
