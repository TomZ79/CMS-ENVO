          ...
          <table class="table table-striped table-hover">
            ...
            <tr>
              ...
              <th>
                <button type="submit" name="delete" id="button_delete" class="btn btn-danger btn-xs"
                        onclick="if(!confirm('<?php echo $tlt["st"]["co"]; ?>'))return false;"><i
            ...
            <?php if (isset($JAK_TICKETCOM_ALL) && is_array($JAK_TICKETCOM_ALL)) foreach ($JAK_TICKETCOM_ALL as $v) { ?>
              <tr>
                ...
                <td><?php if (isset($JAK_TICKET_ALL) && is_array($JAK_TICKET_ALL)) foreach ($JAK_TICKET_ALL as $z) {
                    if ($v["ticketid"] == $z["id"]) { ?><a
                      href="index.php?p=ticketing&amp;sp=comment&amp;ssp=sort&amp;sssp=ticketid&amp;ssssp=<?php echo $z["id"]; ?>"><?php echo $z["title"]; ?></a><?php }
                  } ?></td>
                <td><?php if ($v["userid"] == '0') { ?><?php echo $tl["general"]["g28"]; ?><?php } else { ?><a
                    href="index.php?p=ticketing&amp;sp=comment&amp;ssp=sort&amp;sssp=user&amp;ssssp=<?php echo $v["userid"]; ?>"><?php echo $v["username"]; ?></a><?php } ?>
                </td>
                <td><a href="index.php?p=ticketing&amp;sp=comment&amp;ssp=approve&amp;sssp=<?php echo $v["id"]; ?>"
                       class="btn btn-default btn-xs"><i
                      class="fa fa-<?php if ($v["approve"] == 0) { ?>lock<?php } else { ?>check<?php } ?>"></i></a></td>
                <td><a href="index.php?p=ticketing&amp;sp=comment&amp;ssp=delete&amp;sssp=<?php echo $v["id"]; ?>"
