...
<html>
<head>
  <title><?=$tlsedi["siteedit_uninstall"]["seuninst"]?></title>
  <meta charset="utf-8">
  <!-- BEGIN Vendor CSS-->
  <?php
  ...
  <div class="row">
    <div class="col-sm-12 m-t-20">
      <div class="jumbotron bg-master pt-1 pl-3 pb-1 pr-3">
        <h3 class="semi-bold text-white"><?=$tlsedi["siteedit_uninstall"]["seuninst"]?></h3>
      </div>
      <hr>
      <div id="notificationcontainer"></div>
      <div class="m-b-30">

        <h4 class="semi-bold"><?=$tlsedi["siteedit_uninstall"]["seuninst1"]?></h4>

        <div data-pages="card" class="card card-transparent" id="card-basic">
          <div class="card-header separator">
            <div class="card-title"><?=$tlsedi["siteedit_uninstall"]["seuninst2"]?></div>
      ...
      <?php if (isset($_POST['uninstall'])) {
          ...
          <script>
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlsedi["siteedit_uninstall"]["seuninst3"]?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
        ...
        <?php } else { ?>
          <div>
            <h5 class="text-danger bold"><?=$tlsedi["siteedit_uninstall"]["seuninst4"]?></h5>
          </div>
          <script>
            $(document).ready(function () {
              'use strict';
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlsedi["siteedit_uninstall"]["seuninst4"]?>',
                position: 'top',
                timeout: 0,
                type: 'danger'
              }).show();
      ...
      <?php if (!$succesfully) { ?>
        <form name="company" action="uninstall.php" method="post" enctype="multipart/form-data">
          <div class="form-group form-inline">
            <label for="text"><?=$tlsedi["siteedit_uninstall"]["seuninst5"]?></label>
          ...
          <button type="submit" name="uninstall" class="btn btn-complete btn-block">
            <?=$tlsedi["siteedit_uninstall"]["seuninst6"]?>
          </button>
        </form>
      <?php } ?>
