                ...
                <textarea class="jakEditorLight" id="jakEditor" rows="4" name="jak_description"><?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
									...
									<div class="col-md-7">
										<div class="radio">
											<label class="checkbox-inline">
												<input type="radio" name="jak_smpt" value="0"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
													if ($v["varname"] == 'nlsmtp_mail' && $v["value"] == '0') { ?> checked="checked"<?php }
												} ?> /> <?php echo $tlnl["nletter"]["d16"]; ?>
											</label>
											<label class="checkbox-inline">
												<input type="radio" name="jak_smpt" value="1"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
													if ($v["varname"] == 'nlsmtp_mail' && $v["value"] == '1') { ?> checked="checked"<?php }
												} ?> /> <?php echo $tlnl["nletter"]["d17"]; ?>
											</label>
										</div>
									</div>
								</div>
								...
								<div class="row-form">
									...
									<div class="col-md-7">
										<div class="radio">
											<label class="checkbox-inline">
												<input type="radio" name="jak_alive" value="1"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
													if ($v["varname"] == 'nlsmtp_alive' && $v["value"] == '1') { ?> checked="checked"<?php }
												} ?> /> <?php echo $tl["general"]["g18"]; ?>
											</label>
											<label class="checkbox-inline">
												<input type="radio" name="jak_alive" value="0"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
								...
								<div class="row-form">
									...
									<div class="col-md-7">
										<div class="radio">
											<label class="checkbox-inline">
												<input type="radio" name="jak_auth" value="1"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
													if ($v["varname"] == 'nlsmtp_auth' && $v["value"] == '1') { ?> checked="checked"<?php }
												} ?> /> <?php echo $tl["general"]["g18"]; ?>
											</label>
											<label class="checkbox-inline">
												<input type="radio" name="jak_auth" value="0"<?php if (isset($JAK_SETTING) && is_array ($JAK_SETTING)) foreach ($JAK_SETTING as $v) {
