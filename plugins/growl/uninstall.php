...
<html>
<head>
  <title><?php echo $tlgwl["gwl_uninstall"]["gwluninst"]; ?></title>
  <meta charset="utf-8">
  <!-- BEGIN Vendor CSS-->
  ...
  <div class="row">
    <div class="col-md-12 m-t-20">
      <div class="jumbotron bg-master">
        <h3 class="semi-bold text-white"><?php echo $tlgwl["gwl_uninstall"]["gwluninst"]; ?></h3>
      </div>
      <hr>
      <div id="notificationcontainer"></div>
      <div class="m-b-30">
        <h4 class="semi-bold"><?php echo $tlgwl["gwl_uninstall"]["gwluninst1"]; ?></h4>

        <div id="portlet-advance" class="panel panel-transparent">
          <div class="panel-heading separator">
            <div class="panel-title"><?php echo $tlgwl["gwl_uninstall"]["gwluninst2"]; ?></div>
      ...
      <?php if (isset($_POST['uninstall'])) {
        ...
        if (isset($_POST["captcha"]) && $_POST["captcha"] != "" && $_SESSION["code"] == $_POST["captcha"]) {
          ...
          if ($rows) {
            ...
            $envodb->query('DELETE FROM ' . DB_PREFIX . 'plugins WHERE name = "Growl"');
            $envodb->query('DELETE FROM ' . DB_PREFIX . 'pluginhooks WHERE product = "growl"');

            /* Remove tables with data */
            $envodb->query('DROP TABLE ' . DB_PREFIX . 'growl');
          ...
          <script>
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tlgwl["gwl_uninstall"]["gwluninst3"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
        ...
        <?php } else { ?>
          <div>
            <h5 class="text-danger bold"><?php echo $tlgwl["gwl_uninstall"]["gwluninst4"]; ?></h5>
          </div>
          <script>
            $(document).ready(function () {
              'use strict';
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tlgwl["gwl_uninstall"]["gwluninst4"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'danger'
              }).show();
      ...
      if (!$succesfully) { ?>
        <form name="company" action="uninstall.php" method="post" enctype="multipart/form-data">
          <div class="form-group form-inline">
            <label for="text"><?php echo $tlgwl["gwl_uninstall"]["gwluninst5"]; ?></label>
            <input type="text" name="captcha" class="form-control" id="text">
            <img src="../../assets/plugins/captcha/simple/captcha.php" class="m-l-10"/>
          </div>
          <button type="submit" name="uninstall" class="btn btn-complete btn-block"><?php echo $tlgwl["gwl_uninstall"]["gwluninst6"]; ?></button>
        </form>
      <?php } ?>
