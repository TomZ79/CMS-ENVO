...
<head>
  ...
  <title><?php echo $jkv["title"]; ?> - <?php if ($page) {
      echo ucfirst($page);
    } ?></title>
    ...
    <div class="row">
      <div class="col-xs-12">
        <h2 class="page-header">
          ...
          <small class="pull-right"><?php echo $tl["page"]["p2"]; ?>
            : <?php echo date($jkv["shopdateformat"] . $jkv["shoptimeformat"], strtotime($row["time"])); ?></small>
        </h2>
      </div><!-- /.col -->
    </div>
    <!-- info row -->
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
        ...
        <address>
          ...
          echo $row["name"] . '<br />' . $row["address"] . '<br />' . $row["city"] . ' ' . $row["zip_code"] . '<br />' . $JAK_COUNTRY; ?>
        </address>
      </div><!-- /.col -->
      <div class="col-sm-4 invoice-col">
        ...
        <address>
          <?php if ($row['sh_name']) {
            if ($row["sh_company"]) {
              echo $row["sh_company"] . '<br />';
            }
            echo $row["sh_name"] . '<br />' . $row["sh_address"] . '<br />' . $row["sh_city"] . ' ' . $row["sh_zip_code"] . '<br />' . $JAK_SHCOUNTRY;
        ...
        <table class="table table-striped">
          ...
          <?php if (isset($jak_ordered) && is_array($jak_ordered)) foreach ($jak_ordered as $jo) { ?>
            ...
            <tr<?php if (!$row["paid"] == 1) { ?> class="unpaid"<?php } ?>>
              ...
              <td><?php echo $jo["title"]; ?><?php if ($jo["product_option"]) {
                  echo '&nbsp;(' . $jo["product_option"] . ')&nbsp;';
                } ?></td>
              <td><span><?php echo $jo["price"]; ?>&nbsp;<?php echo $row["currency"];
                  if ($jo["coupon_price"] != "0.00" && $jo["price"] != $jo["coupon_price"]) {
                    echo ' (' . $tlec["shop"]["m86"] . $jo["coupon_price"] . '&nbsp;' . $row["currency"] . ')';
                  } ?></span></td>
            </tr>
        ...
        <div class="table-responsive">
          <table class="table">
            ...
            <tr>
              ...
              <td><?php if ($row["freeshipping"]) {
                  echo $tl["shop"]["m81"];
                } else {
                  echo $row["shipping"] . ' ' . $row["currency"];
                } ?></td>
            </tr>
            <tr>
