...
if (file_exists(APP_PATH . 'plugins/digital_house/admin/lang/' . $site_language . '.ini')) {
  $tldh = parse_ini_file(APP_PATH . 'plugins/digital_house/admin/lang/' . $site_language . '.ini', TRUE);
} else {
  $tldh = parse_ini_file(APP_PATH . 'plugins/digital_house/admin/lang/en.ini', TRUE);
}
...
<html>
<head>
  <title><?php echo $tldh["dh_install"]["dhinst"]; ?></title>
  <meta charset="utf-8">
  <!-- BEGIN Vendor CSS-->
    ...
    <div class="col-md-12">
      <div class="col-md-12 m-t-20">
        <div class="jumbotron bg-master">
          <h3 class="semi-bold text-white"><?php echo $tldh["dh_install"]["dhinst"]; ?></h3>
        </div>
        <hr>
        <div id="notificationcontainer"></div>
        <div class="m-b-30">
          <h4 class="semi-bold"><?php echo $tldh["dh_install"]["dhinst1"]; ?></h4>

          <div id="portlet-advance" class="panel panel-transparent">
            <div class="panel-heading separator">
              <div class="panel-title"><?php echo $tldh["dh_install"]["dhinst2"]; ?></div>
        ...
        */
        $jakdb->query('SELECT id FROM ' . DB_PREFIX . 'plugins WHERE name = "digital_house"');
          ...
          <script>
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tldh["dh_install"]["dhinst3"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'warning'
              }).show();
        ...
        $results = $jakdb->query('SELECT id FROM ' . DB_PREFIX . 'plugins WHERE name = "Digital_house"');
        ...
        $adminlang = 'if (file_exists(APP_PATH.\'plugins/digital_house/admin/lang/\'.$site_language.\'.ini\')) {
  $tldh = parse_ini_file(APP_PATH.\'plugins/digital_house/admin/lang/\'.$site_language.\'.ini\', true);
} else {
  $tldh = parse_ini_file(APP_PATH.\'plugins/digital_house/admin/lang/en.ini\', true);
}';
        ...
        $sitelang = 'if (file_exists(APP_PATH.\'plugins/digital_house/lang/\'.$site_language.\'.ini\')) {
  $tldh = parse_ini_file(APP_PATH.\'plugins/digital_house/lang/\'.$site_language.\'.ini\', true);
} else {
  $tldh = parse_ini_file(APP_PATH.\'plugins/digital_house/lang/en.ini\', true);
}';
        ...
        $insertphpcode = 'if (isset($defaults[\'jak_digitalhouse\'])) {
	$insert .= \'digitalhouse = \"\'.$defaults[\'jak_digitalhouse\'].\'\",\'; }';

        // EN: Insert data to table 'pluginhooks'
        // CZ: Vložení potřebných dat to tabulky 'pluginhooks'
        $jakdb->query('INSERT INTO ' . DB_PREFIX . 'pluginhooks (`id`, `hook_name`, `name`, `phpcode`, `product`, `active`, `exorder`, `pluginid`, `time`) VALUES
(NULL, "php_admin_lang", "Digital House Admin Language", "' . $adminlang . '", "digitalhouse", 1, 1, "' . $rows['id'] . '", NOW()),
(NULL, "php_lang", "Digital House Site Language", "' . $sitelang . '", "digitalhouse", 1, 1, "' . $rows['id'] . '", NOW()),
(NULL, "tpl_admin_usergroup", "Digital House Usergroup New", "plugins/digital_house/admin/template/usergroup_new.php", "digitalhouse", 1, 1, "' . $rows['id'] . '", NOW()),
(NULL, "tpl_admin_usergroup_edit", "Digital House Usergroup Edit", "plugins/digital_house/admin/template/usergroup_edit.php", "digitalhouse", 1, 1, "' . $rows['id'] . '", NOW()),
(NULL, "php_admin_usergroup", "Digital House Usergroup SQL", "' . $insertphpcode . '", "digitalhouse", 1, 1, "' . $rows['id'] . '", NOW())');

        // EN: Insert data to table 'setting'
        // CZ: Vložení potřebných dat to tabulky 'setting'
        $jakdb->query('INSERT INTO ' . DB_PREFIX . 'setting (`varname`, `groupname`, `value`, `defaultvalue`, `optioncode`, `datatype`, `product`) VALUES
("digitalhousetitle", "digitalhouse", "Digital House", "Digital House", "input", "free", "digitalhouse")');

        // EN: Insert data to table 'usergroup'
        // CZ: Vložení potřebných dat to tabulky 'usergroup'
        $jakdb->query('ALTER TABLE ' . DB_PREFIX . 'usergroup ADD `digitalhouse` SMALLINT(1) UNSIGNED NOT NULL DEFAULT 0 AFTER `advsearch`');
...
(NULL, "Digital House", "digital-house", NULL, 1, 0, 5, 0, 0, 1, "' . $rows['id'] . '")');

        // EN: Create table for plugin (cities)
        // CZ: Vytvoření tabulky pro plugin (města)
        $jakdb->query('CREATE TABLE IF NOT EXISTS ' . DB_PREFIX . 'digitalhousecities (
  ...
  PRIMARY KEY (`id`)
        ...
        ?>
          ...
          <script>
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tldh["dh_install"]["dhinst4"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
        ...
        ?>
          ...
          <div class="alert bg-danger"><?php echo $tldh["dh_install"]["dhinst5"]; ?></div>
          <form name="company" method="post" action="uninstall.php" enctype="multipart/form-data">
            <button type="submit" name="redirect" class="btn btn-danger btn-block"><?php echo $tldh["dh_install"]["dhinst6"]; ?></button>
          </form>
        ...
        if (!$succesfully) { ?>
          <form name="company" method="post" action="install.php">
            <button type="submit" name="install" class="btn btn-complete btn-block"><?php echo $tldh["dh_install"]["dhinst7"]; ?></button>
          </form>
        <?php }
        } ?>
