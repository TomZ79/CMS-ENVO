  ...
  <li class="jakcontent">
    ...
    <div class="form-group">
      ...
      <select name="jak_showslider" class="form-control">
        <option
          value="0"<?php if (isset($_REQUEST["jak_showslider"]) && $_REQUEST["jak_showslider"] == '0' || !isset($_REQUEST["jak_showslider"])) { ?> selected="selected"<?php } ?>><?php echo $tl["cform"]["c18"]; ?></option>
        <?php if (isset($JAK_GET_SLIDER) && is_array($JAK_GET_SLIDER)) foreach ($JAK_GET_SLIDER as $las) { ?>
          <option value="<?php echo $las["id"]; ?>"<?php if (isset($_REQUEST["jak_showslider"]) && $las["id"] == $_REQUEST["jak_showslider"]) { ?> selected="selected"<?php } ?>><?php echo $las["title"]; ?></option><?php } ?>
      </select>
    </div>
