...
if (isset($_FILES['file'])) {
  ...
  if (in_array($ext, $valid_extensions)) {
    ...
    if (move_uploaded_file($tmp, $pathfull)) {
      ...
      $envodb->query('INSERT ' . DB_PREFIX . 'int_housedocu SET id = NULL, houseid = "' . $_REQUEST['houseID'] . '", description = "", filename = "' . $filename . '", fullpath = "' . $fullpath . '", timedefault = NOW(), timeedit = NOW()');

      // Get all files for house
      $result = $envodb->query('SELECT * FROM ' . DB_PREFIX . 'int_housedocu WHERE houseid = "' . $_REQUEST['houseID'] . '" ORDER BY id ASC');
