...
<html>
<head>
	<title><?= $tlw["wiki_uninstall"]["wikiuninst"] ?></title>
	<meta charset="utf-8">
	<!-- BEGIN Vendor CSS-->
	<?php
	...
	<div class="row">
		<div class="col-sm-12 m-t-20">
			<div class="jumbotron bg-master pt-1 pl-3 pb-1 pr-3">
				<h3 class="semi-bold text-white"><?= $tlw["wiki_uninstall"]["wikiuninst"] ?></h3>
			</div>
			<hr>
			<div id="notificationcontainer"></div>
			<div class="m-b-30">

				<h4 class="semi-bold"><?= $tlw["wiki_uninstall"]["wikiuninst1"] ?></h4>

				<div data-pages="card" class="card card-transparent" id="card-basic">
					<div class="card-header separator">
						<div class="card-title"><?= $tlw["wiki_uninstall"]["wikiuninst2"] ?></div>
			...
			<?php if (isset($_POST['uninstall'])) {
				...
				if (isset($_POST["captcha"]) && $_POST["captcha"] != "" && $_SESSION["code"] == $_POST["captcha"]) {
					...
					if ($rows) {
						...
						$envodb -> query('DELETE FROM ' . DB_PREFIX . 'plugins WHERE name = "WIKI"');
						...
						$envodb -> query('DELETE FROM ' . DB_PREFIX . 'pluginhooks WHERE product = "wiki"');
						...
						$envodb -> query('ALTER TABLE ' . DB_PREFIX . 'pagesgrid DROP wikiid');
            ...
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlw["wiki_uninstall"]["wikiuninst3"]?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
              ...
              e.preventDefault();
					...
					<h5 class="text-danger bold"><?= $tlw["wiki_uninstall"]["wikiuninst4"] ?></h5>
				</div>
					<script>
            $(document).ready(function () {
              'use strict';
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlw["wiki_uninstall"]["wikiuninst4"]?>',
                position: 'top',
                timeout: 0,
                type: 'danger'
              }).show();
			...
			if (!$succesfully) { ?>
				<form name="company" action="uninstall.php" method="post" enctype="multipart/form-data">
					<div class="form-group form-inline">
						<label for="text"><?= $tlw["wiki_uninstall"]["wikiuninst5"] ?></label>
					...
					<button type="submit" name="uninstall" class="btn btn-complete btn-block">
						<?= $tlw["wiki_uninstall"]["wikiuninst6"] ?>
					</button>
				</form>
			<?php } ?>
