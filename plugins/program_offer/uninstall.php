...
<html>
<head>
  <title><?php echo $tlpo["po_uninstall"]["pouninst"]; ?></title>
  <meta charset="utf-8">
  <!-- BEGIN Vendor CSS-->
  ...
  <div class="row">
    <div class="col-md-12 m-t-20">
      <div class="jumbotron bg-master">
        <h3 class="semi-bold text-white"><?php echo $tlpo["po_uninstall"]["pouninst"]; ?></h3>
      </div>
      <hr>
      <div id="notificationcontainer"></div>
      <div class="m-b-30">
        <h4 class="semi-bold"><?php echo $tlpo["po_uninstall"]["pouninst1"]; ?></h4>

        <div id="portlet-advance" class="panel panel-transparent">
          <div class="panel-heading separator">
            <div class="panel-title"><?php echo $tlpo["po_uninstall"]["pouninst2"]; ?></div>
      ...
      <?php if (isset($_POST['uninstall'])) {
        ...
        if (isset($_POST["captcha"]) && $_POST["captcha"] != "" && $_SESSION["code"] == $_POST["captcha"]) {
          ...
          if ($rows) {
            ...
            $jakdb->query('DELETE FROM ' . DB_PREFIX . 'plugins WHERE name = "Program_offer"');
            $jakdb->query('DELETE FROM ' . DB_PREFIX . 'pluginhooks WHERE product = "programoffer"');
            $jakdb->query('DELETE FROM ' . DB_PREFIX . 'setting WHERE product = "programoffer"');
            $jakdb->query('ALTER TABLE ' . DB_PREFIX . 'usergroup DROP `programoffer`');
            $jakdb->query('DELETE FROM ' . DB_PREFIX . 'categories WHERE pluginid = "' . smartsql($rows['id']) . '"');

            /* Remove talbes */
            $jakdb->query('DROP TABLE ' . DB_PREFIX . 'programoffertvprogram');
            $jakdb->query('DROP TABLE ' . DB_PREFIX . 'programoffertvchannel');
            $jakdb->query('DROP TABLE ' . DB_PREFIX . 'programoffertvtower');
          ...
          <script>
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tlpo["po_uninstall"]["pouninst3"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
        ...
        <?php } else { ?>
          <div>
            <h5 class="text-danger bold"><?php echo $tlpo["po_uninstall"]["pouninst4"]; ?></h5>
          </div>
          <script>
            $(document).ready(function () {
              'use strict';
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?php echo $tlpo["po_uninstall"]["pouninst4"]; ?>',
                position: 'top',
                timeout: 0,
                type: 'danger'
              }).show();
      ...
      if (!$succesfully) { ?>
        <form name="company" action="uninstall.php" method="post" enctype="multipart/form-data">
          <div class="form-group form-inline">
            <label for="text"><?php echo $tlpo["po_uninstall"]["pouninst5"]; ?></label>
            <input type="text" name="captcha" class="form-control" id="text">
            <img src="../../assets/plugins/captcha/simple/captcha.php" class="m-l-10"/>
          </div>
          <button type="submit" name="uninstall" class="btn btn-complete btn-block"><?php echo $tlpo["po_uninstall"]["pouninst6"]; ?></button>
        </form>
      <?php } ?>
