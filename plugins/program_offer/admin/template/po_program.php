...
<div class="savebutton-medium hidden-xs">
  ...
  echo $Html->addAnchor('index.php?p=program-offer&amp;sp=tvprogram&amp;ssp=newprogram', $tlpo["po_button"]["pobtn2"], '', 'btn btn-info button');
  ?>
        ...
        <?php foreach ($JAK_TVPROGRAM_ALL as $tp) { ?>
          <tr>
            ...
            <td>
              ...
              <?php
              // Add Html Element -> addAnchor (Arguments: href_link, text, id, class, optional assoc. array)
              echo $Html->addAnchor('index.php?p=program-offer&amp;sp=tvprogram&amp;ssp=editprogram&amp;id=' . $tp["id"], envo_cut_text($tp["name"], 70, '...'));
              ?>
              ...
              if ($tp["channelid"] != '0') {
                if (isset($JAK_TVTOWER_ALL) && is_array($JAK_TVTOWER_ALL)) foreach ($JAK_TVTOWER_ALL as $tt) {
                  if (in_array($tt["id"], explode(',', $tp["towerid"]))) {
                    // Add Html Element -> addAnchor (Arguments: href_link, text, id, class, optional assoc. array)
                    echo $Html->addAnchor('index.php?p=program-offer&amp;sp=tvtower&amp;ssp=edittvtower&amp;id=' . $tp["towerid"], envo_cut_text($tt["name"], 70, '...'));
                  }
                }
              } else {
                echo 'Archiv';
              ...
              if ($tp["channelid"] != '0') {
                if (isset($JAK_TVCHANNEL_ALL) && is_array($JAK_TVCHANNEL_ALL)) foreach ($JAK_TVCHANNEL_ALL as $tc) {
                  if (in_array($tc["id"], explode(',', $tp["channelid"]))) {
                    // Add Html Element -> addAnchor (Arguments: href_link, text, id, class, optional assoc. array)
                    echo $Html->addAnchor('index.php?p=program-offer&amp;sp=tvchannel&amp;ssp=editchannel&amp;id=' . $tp["channelid"], $tc["number"] . ' K');
                  }
                }
              } else {
                echo 'Archiv';
