 ...
 * =============================================================================
 * Web: Rafael Staib (http://www.jquery-steps.com)
 */
!function (a, b) {
  function c(a, b) {
    o(a).push(b)
...
$(function () {
  ...
  $("#xmlseo-wizard").steps({
    ...
    onStepChanging: function (event, currentIndex, newIndex) {
      ...
      if (newIndex === 1) {
        // Show Sitemap path in 'pre'
        var baseurl = window.location.origin;
        var path = $("input[name=jak_xmlseopath]").val();
        ...
        $('.code').html("Sitemap: " + baseurl + "/" + path + lastCHar + "sitemap.xml");
      }
      // Allways allow next action even if the current form is not valid!
      if (currentIndex < newIndex) {
        return true;
      }
