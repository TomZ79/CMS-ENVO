...
if (isset($_POST['action']) && $_POST['action'] == 'form1') {
  ...
  if (!is_dir(APP_PATH . $path)) {
    $error1 = "Directory does not exist: " . BASE_URL_ORIG . $path;
  } else if (!is_writable(APP_PATH . $path)) {
    $error2 = "Directory not writable: " . BASE_URL_ORIG . $path;
  } else {
    $succes1 = "Directory is set to: " . BASE_URL_ORIG . $path . "<br>Data was entered into the file robots.txt<br>Created backup file robots.txt.backup";
    // Create backup file
  ...
  $error  = $error;
} else if (isset($_POST['action']) && $_POST['action'] == 'form3') {
  // Form 3 - Show/Hide Sitemap.xml
  ...
  } else if (isset($_POST['submit_two'])) {
