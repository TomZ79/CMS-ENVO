  ...
  <li class="jakcontent">
    ...
    <div class="form-group">
      ...
      <select name="jak_showgallery[]" multiple="multiple" class="form-control">
        <option
          value="0"<?php if (isset($JAK_FORM_DATA["showgallery"]) && $JAK_FORM_DATA["showgallery"] == 0) { ?> selected="selected"<?php } ?>><?php echo $tl["cform"]["c18"]; ?></option>
        <?php if (isset($JAK_GET_GALLERY) && is_array($JAK_GET_GALLERY)) foreach ($JAK_GET_GALLERY as $gal) { ?>
          <option
            value="<?php echo $gal["id"]; ?>"<?php if (isset($JAK_FORM_DATA["showgallery"]) && $JAK_FORM_DATA["showgallery"] == $gal["id"]) { ?> selected="selected"<?php } ?>><?php echo $gal["title"]; ?></option>
        <?php } ?>
      </select>
    </div>
