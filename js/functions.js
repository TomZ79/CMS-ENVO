...
!function (t) {
	var o = function (o, i) {
		var s = {
			...
			newTab: !0,
			showTooltips: !1,
			tooltipsterOptions: {theme: "sl-tooltip-black"},
			tooltipClass: "sl-tooltip",
			showTooltips: !1,
			tooltips: {googleplus: "google+"},
			itemClass: "",
			itemCss: {}
		}
		this.elem = i, this.options = t.extend({}, s, o), this.options.tooltips = t.extend({}, s.tooltips, this.options.tooltips), (this.options.theme || this.themeDirPath) && this.options.profiles && this.elem && (this.hoverFunctions = {
			myFadeOut: {
				inFunc: function (t) {
					t.fadeTo(.2, .5)
				}, outFunc: function (t) {
	...
	t.extend(o.prototype, {
		go: function () {
			this.setHoverFucntions(), $container = t("<" + this.options.containerTag + "></" + this.options.containerTag + ">")
			...
			$container.children().last().css("margin-right", 0), this.elem.append($container), this.options.showTooltips && t("." + this.options.tooltipClass).tooltipster(this.options.tooltipsterOptions)
		}, itemElement: function (o) {
			var i = t("<" + this.options.elementTag + "></" + this.options.elementTag + ">")
			i.addClass("sl-item"), i.addClass(this.options.itemClass), this.options.hoverInFunction && this.options.hoverOutFunction ? i.hover(function () {
				n.options.hoverInFunction(t(this))
			}, function () {
				n.options.hoverOutFunction(t(this))
			}) : i.addClass(this.hoverClass())
			var s = t.extend({}, {"margin-right": this.options.pixelsBetweenItems}, this.options.itemCss)
			i.css(s)
			var n = this, e = t("<a></a>")
			if (e.attr("href", this.options.profiles[o]), e.height(this.options.size), e.width(this.options.size), this.options.themeDirPath) {
				var h = t('<img src = "' + this.socialIconPath(o) + "." + this.options.iconFileExtension + '"/>')
				e.append(h)
			} else e.addClass(o), e.addClass(this.options.theme)
			return this.options.newTab && e.attr("target", "_blank"), this.options.showTooltips && (e.addClass(this.options.tooltipClass), e.attr("title", this.tooltip(o)), e.addClass(this.options.tooltipsTheme)), i.append(e), i
		}, hoverClass: function () {
			return "fadeOut" == this.options.hoverEffect ? "" : this.options.hoverEffect
		}, socialIconPath: function (t) {
			return this.options.themeDirPath + "/" + t
		}, setHoverFucntions: function () {
			this.options.hoverOutFunction && this.options.hoverInFunction || "myFadeOut" == this.options.hoverEffect && (this.options.hoverInFunction = this.hoverFunctions[this.options.hoverEffect].inFunc, this.options.hoverOutFunction = this.hoverFunctions[this.options.hoverEffect].outFunc)
		}, tooltip: function (t) {
			return this.options.tooltips.hasOwnProperty(t) ? this.options.tooltips[t] : t
		}
	}), t.fn.sollist = function (i) {
		this.each(function () {
			var s = t(this)
			new o(i, s)
		})
