...
// Check For An Update
$getVersions = @file_get_contents('http://update.jakweb.ch/cms_update/current_release.php') or die('<div class="alert bg-danger">Cannot access the release file.</div>');
	...
	foreach ($versionList as $aV) {
		...
		if ($aV > $jkv["version"]) {
			...
			if (!is_file(APP_PATH.JAK_FILES_DIRECTORY.'/updates/part_cms_'.$dlpackage.'.zip' )) {
				...
				$newUpdate = @file_get_contents('http://update.jakweb.ch/cms_update/part_cms_'.$dlpackage.'.zip') or die('<div class="alert bg-danger">Cannot access the latest update.</div>');
