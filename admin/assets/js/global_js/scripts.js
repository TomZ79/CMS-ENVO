...
$.AdminEnvo.options = {
  ...
  BSTooltipSelector: '[data-toggle="tooltipEnvo"]',
...
$(function () {
  ...
  $('[data-confirm-control]').click(function (e) {
    ...
    var links = $(this).attr('href');
    e.preventDefault();
    var html = '<p class="m-t-15">Z důvodu ochrany opište následující text</p><p>OPRAVDU CHCI ODSTRANIT ZÁZNAM Z DATABÁZE</p><input type="text" id="text-control" name="text-control" class="m-t-15" style="width:100%;padding:5px;" >';

    // Show Message
    bootbox.setLocale(o.BootboxLang);
    bootbox.confirm({
      title: 'Potvrzení o odstranění!',
      message: $(this).attr('data-confirm-control') + html,
      className: 'bootbox-confirm-del',
      animate: true,
      buttons: {
        confirm: {
  ...
  $('#button_truncate').click(function (e) {
    // Init
    var links = $(this).attr('href');
    e.preventDefault();
    // Show Message
...
/** FUNCTION - Insert block and restore content
 * @require: Without external plugin
 ========================================================================*/
...
function insert_code_admin_member_guest () {
  return '{{if admin}}\n\n For Admin \n\n{{endif}}\n\n\n{{if members}}\n\n For Members \n\n{{endif}}\n\n\n{{if notmembers}}\n\n For Guest \n\n{{endif}}\n\n';
}

function insert_code_admin () {
  return '{{if admin}}\n\n For Admin \n\n{{endif}}\n\n';
}
...
function restoreContent (fieldname, backupid, id) {
  ...
  $.ajax({
    ...
    beforeSend: function (x) {
      $('#restorediv').css('visibility', 'visible');
    },
    success: function (data) {
      ...
      if (data.status == 'success') {
        ...
        setTimeout(function () {
          $('#restorediv').css('visibility', 'hidden');
        }, 2000);
...
$(function () {
  ...
  $('.table-expandable').each(function () {
    ...
    table.children('tbody').children('tr').filter(':even').each(function () {
      var element = $(this);
      element.append('<td style="text-align: center;vertical-align: middle;"><div class="table-expandable-arrow"></div></td>');
    });
  });
...
/** BOOSTRAP TABS - keep current tab after page reload
...
/* JQUERY PASSY - Generating and analazing passwords, realtime.
 ========================================================================*/
/*
