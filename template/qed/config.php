	...
	if (isset($menu['parents'][ $parent ])) {
		...
		foreach ($menu['parents'][ $parent ] as $itemId) {
			...
			if (!isset($menu['parents'][ $itemId ])) {
				$html .= '<li' . ($active == $menu["items"][ $itemId ]["pagename"] ? ' class="active"' : '') . '><a href="' . $menu["items"][ $itemId ]["varname"] . '"' . ($active == $menu["items"][ $itemId ]["pagename"] ? ' class="active"' : '') . '>' . ($menu["items"][ $itemId ]["catimg"] ? '<i class="fa ' . $menu["items"][ $itemId ]["catimg"] . '"></i> ' : '') . $menu["items"][ $itemId ]["name"] . '</a></li>';
			}

			// IF MENU HAS SUBMENU
			// Není řešen případ kdy $menu['parents'][$itemId] je rovno NULL, tento případ nastává pro titulní HOMEPAGE
			// Tudíž nelze přidat class="active" do HOME linku
			if (isset($menu['parents'][ $itemId ])) {
