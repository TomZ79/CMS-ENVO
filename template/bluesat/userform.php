...
<form role="form" class="jak-ajaxform cFrom" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
  ...
  <?php if (!JAK_USERID) { ?>
    ...
    <div class="form-group">
      ...
      <input type="text" name="co_url" id="co_url" class="form-control"
             value="<?php if (isset($_REQUEST["url"])) echo $_REQUEST["url"]; ?>"/>
    </div>
  ...
  <div class="form-group">
    ...
    <textarea name="userpost" class="form-control userpost" id="userpost"
              rows="8"><?php if (isset($_REQUEST["userpost"])) echo $_REQUEST["userpost"]; ?></textarea>
  </div>
