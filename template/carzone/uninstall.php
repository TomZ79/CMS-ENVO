...
<html lang="en">
<head>
  <title><?=$tl["uninstalltemplate"]["unitpl"] . ' - CARZONE Template'?></title>
  <meta charset="utf-8">
  <!-- BEGIN Vendor CSS-->
  ...
  <div class="row">
    <div class="col-sm-12 m-t-20">
      <div class="jumbotron bg-master pt-1 pl-3 pb-1 pr-3">
        <h3 class="semi-bold text-white"><?=$tl["uninstalltemplate"]["unitpl"] . ' - CARZONE Template'?></h3>
      </div>
      <hr>
      ...
      <?php if (isset($_POST['uninstall'])) {
        ...
        if (isset($_POST["captcha"]) && $_POST["captcha"] != "" && $_SESSION["code"] == $_POST["captcha"]) {
          // Delete all settings
          $envodb->query('DELETE FROM ' . DB_PREFIX . 'setting WHERE product = "tpl_carzone"');

          // Delete php code for lang site from hooks
          $envodb->query('DELETE FROM ' . DB_PREFIX . 'pluginhooks WHERE product = "tpl_carzone"');
