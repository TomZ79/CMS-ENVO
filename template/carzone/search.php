        ...
        <?php } else { ?>
          ...
          <form role="form" action="<?= $P_SEAERCH_LINK ?>" method="post">
            <div class="input-group">
              <input type="text" name="envoSH" id="Jajaxs" class="form-control" placeholder="<?= $tl["placeholder"]["plc"];
              if ($setting["fulltextsearch"]) echo $tl["placeholder"]["plc1"]; ?>">
              <span class="input-group-btn">
                <button type="submit" class="site-button" name="search" id="JajaxSubmitSearch"><?= $tl["searching"]["stxt"] ?></button>
              </span>
            </div>
        ...
        if (isset($ENVO_SEARCH_USED)) { ?>
          ...
          <div class="col-md-12">
            ...
            <div class="col-md-6">
              ...
              if (isset($count)) {
                echo '<p class="pull-right" style="margin: 20px 0;">' . str_replace("%s", $count, $tl["searching"]["stxt2"]) . '</p>';
              } else {
                echo '<div class="alert alert-danger">' . $tl["searching_error"]["ser2"] . '</div>';
              }
