<?php if (ENVO_SEARCH && ENVO_USER_SEARCH && $page != 'search') { ?>
  ...
  <aside class="nav-sidebar hidden-xs">
		...
		<?php if ($setting["ajaxsearch"] && $AJAX_SEARCH_PLUGIN_URL) { ?>
			<div class="row">
				<div class="col-xs-5">
					<div class="hideAdvSearchResult"><a class="btn btn-default btn-xs" href="<?=$P_SEAERCH_LINK?>"><i
								class="fa fa-search"></i> <?=$tl["searching"]["stxt10"]?></a></div>
				</div>
				<div class="col-xs-5">
					<div class="hideSearchResult"><a class="btn btn-warning btn-xs" href="javascript:void(0)"><i
								class="fa fa-remove"></i> <?=$tl["searching"]["stxt11"]?></a></div>
				</div>
				<div class="col-xs-2">
