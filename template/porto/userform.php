...
<form role="form" class="envo-ajaxform cFrom" method="post" action="<?=$_SERVER['REQUEST_URI']?>">
	...
	<?php if (!ENVO_USERID) { ?>
		...
		<div class="form-group">
			<label class="control-label" for="co_name"><?=$tl["contact"]["c1"]?> <i class="fa fa-star"></i></label>
			<input type="text" name="co_name" id="co_name" class="form-control" value="<?php if (isset($_REQUEST["name"])) echo $_REQUEST["name"]; ?>"/>
		</div>
		<div class="form-group">
			<label class="control-label" for="co_email"><?=$tl["contact"]["c2"]?> <i class="fa fa-star"></i></label>
			<input type="text" name="co_email" id="co_email" class="form-control" value="<?php if (isset($_REQUEST["email"])) echo $_REQUEST["email"]; ?>"/>
		</div>
		<div class="form-group">
			<label class="control-label" for="co_url"><?=$tl["contact"]["c17"]?></label>
			<input type="text" name="co_url" id="co_url" class="form-control" value="<?php if (isset($_REQUEST["url"])) echo $_REQUEST["url"]; ?>"/>
		</div>

	<?php } else { ?>

		<input type="hidden" name="co_name" value="<?=$ENVO_USERNAME?>"/>

	<?php } ?>

	<div class="form-group">
		<label class="control-label" for="userpost"><?=$tl["cmsg"]["c7"]?> <i class="fa fa-star"></i></label>
	...
	<input type="hidden" name="uformextraid" id="uformextraid" value="<?=$ENVO_UFORM_EXTRA?>"/>
	<input type="hidden" name="userPost" value="1"/>
	<button type="submit" class="btn btn-color btn-block envo-submit"><?=$tl["button"]["btn3"]?></button>
