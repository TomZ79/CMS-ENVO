<script>
  function fill(title, id) {

    //Assigning value to "ajaxlivesearch" div in "ajaxsearch.php" file.
    $('#ajaxlivesearch').val(title).attr('data-articleid', id);
  ...
  function ajaxshow(value) {
    ...
    if (id != '') {
      window.location.href = '/' + value + '/faq-article/' + id + '/' + val;
    }
    ...
    $('#ajaxlivesearch').keyup(function () {
      ...
      //If name is not empty.
        ...
        $.ajax({
          ...
          url: '/template/porto/plugintemplate/faq/livesearch/ajaxsearch.php',
          //Data type
          datatype: 'json',
