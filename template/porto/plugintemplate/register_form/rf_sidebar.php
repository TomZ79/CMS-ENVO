	...
	<?php if (!ENVO_USERID) {
		...
		<form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
			<div class="form-group<?php if (isset($errorlo["e"])) echo " has-error"; ?>">
				...
				<input type="text" name="envoU" id="username" class="form-control" value="<?php if (isset($_REQUEST["envoU"])) echo $_REQUEST["envoU"]; ?>" placeholder="<?= $tl["login"]["l1"] ?>"/>
			</div>
			<div class="form-group<?php if (isset($errorlo["e"])) echo " has-error"; ?>">
			...
			<div class="form-group">
				...
				<button type="submit" name="login" class="btn btn-success pull-right"><?= $tl["general"]["g146"] ?></button>
				<div class="clearfix"></div>
			</div>
		</form><h4><?= $tl["title"]["t14"] ?></h4>
		...
		<form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
			<div class="form-group<?php if (isset($errorfp["e"])) echo " has-error"; ?>">
				...
				<input type="email" name="envoE" id="email" class="form-control" value="" placeholder="<?= $tl["login"]["l5"] ?>"/>
			</div>
			<button type="submit" name="forgotP" class="btn btn-warning pull-right"><?= $tl["general"]["g178"] ?></button>
			<div class="clearfix"></div>
		</form>

	<?php } else { ?>
		<h4><?= str_replace("%s", $ENVO_USERNAME, $tl["general"]["g8"]) ?></h4><div class="about">
			<!-- Author Photo -->
		...
		</div><p><a href="<?= $P_USR_LOGOUT ?>" class="btn btn-danger pull-right"><?= $tl["title"]["t6"] ?></a>
