		...
		<div class="col-md-6">
			<?php if (!ENVO_USERID) { ?>
				...
				if (!isset($_SESSION["rf_msg_sent"]) || isset($_SESSION["rf_msg_sent"]) && $_SESSION["rf_msg_sent"] != 1) { ?>
					...
					<form method="post" class="cFrom" action="<?= $_SERVER['REQUEST_URI'] ?>" enctype="multipart/form-data">
						<?php if ($setting["rf_simple"]) { ?>
							<div class="form-group<?php if (isset($errorsC["e3"])) echo " has-error"; ?>">
								...
								<input type="text" name="username" id="username" class="form-control" value="<?php if (isset($_REQUEST["username"])) echo $_REQUEST["username"]; ?>" placeholder="<?= $tl["login"]["l1"] ?>">
							</div><div class="form-group<?php if ($errorsC["e4"]) echo " has-error"; ?>">
								<label class="control-label" for="email"><?= $tl["contact"]["c2"] ?> <i class="fa fa-star"></i></label>
								<input type="email" name="email" id="email" class="form-control" value="<?php if (isset($_REQUEST["email"])) echo $_REQUEST["email"]; ?>" placeholder="<?= $tl["contact"]["c2"] ?>">
							</div>
						<?php } else {
						...
						<button type="submit" id="formsubmit" name="registerF" class="btn btn-primary pull-right"><?= $tl["contact"]["s"] ?></button>
						<div class="clearfix"></div>
					</form>
				<?php } ?></div><?php } ?>

			<?php if (ENVO_USERID) { ?>
				<h3><?= sprintf($tl["general"]["g8"], $ENVO_USERNAME) ?></h3><div class="about">
					<!-- Author Photo -->
				...
				<?php if (!ENVO_USERID) { ?>
					...
					<form role="form" method="post" action="<?= $_SERVER['REQUEST_URI'] ?>">
						<div class="form-group<?php if (isset($errorlo)) echo " error"; ?>">
							...
							<input type="text" class="form-control" name="envoU" id="username" value="<?php if (isset($_REQUEST["envoU"])) echo $_REQUEST["envoU"]; ?>" placeholder="<?= $tl["login"]["l22"] ?>">
						</div>
						<div class="form-group<?php if (isset($errorlo)) echo " error"; ?>">
							<label class="control-label" for="password"><?= $tl["login"]["l2"] ?></label>
							<input type="password" class="form-control" name="envoP" id="password" placeholder="<?= $tl["login"]["l2"] ?>">
						</div>
						<div class="checkbox">
							<label>
						...
						<button type="submit" name="login" class="btn btn-primary btn-block"><?= $tl["general"]["g146"] ?></button>
					</form>
					...
					<form role="form" method="post" action="<?= $_SERVER['REQUEST_URI'] ?>">
						...
						<div class="form-group<?php if (isset($errorfp)) echo " error"; ?>">
							...
							<input type="email" class="form-control" name="envoE" id="email" class="form-control" placeholder="<?= $tl["login"]["l5"] ?>">
						</div>
