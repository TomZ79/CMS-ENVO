...
/**
 * TODO! NOVÁ FUNKCE MÍSTO envo_row_exist()
 * EN: Check if row exist
 * CZ:
 *
 * @author  BluesatKV
 ...
 *
 */
function envo_rowExists ($table, $where, $value )
{
	global $envodb;

	// EN: Check if '$value' exists in table -> Build search query to the database
	// CZ:
	$result = $envodb -> query('SELECT ' . $where . ' FROM ' . $table . ' WHERE ' . $where . ' = "' . smartsql($value) . '" LIMIT 1');
