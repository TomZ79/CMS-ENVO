...
<hr><h3 class="text-color"><?= ENVO_PLUGIN_NAME_WIKI ?></h3>

<div class="row">
	<div class="col-md-12 wiki-wrapper">
		<div class="panel-group" id="accordion2">
			<?php if (isset($ENVO_WIKI) && is_array($ENVO_WIKI)) foreach ($ENVO_WIKI as $w) { ?>
				<div class="panel panel-default">
					<div class="panel-heading">
						<a href="<?= $w["parseurl"] ?>"><i class="fa fa-eye"></i></a>
						<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion1" href="#collapse<?= $w["id"] ?>">
							<?= $w["title"] ?>
						</a>
					</div>
					<div id="collapse<?= $w["id"] ?>" class="accordion-body collapse">
						<div class="accordion-inner">
							...
							<div class="pull-right">
								<?php if (ENVO_ASACCESS) { ?>
									...
									<a class="btn btn-default btn-xs quickedit" href="<?= BASE_URL ?>admin/index.php?p=wiki&amp;sp=quickedit&amp;id=<?= $w["id"] ?>" title="<?= $tl["button"]["btn2"] ?>"><i class="fa fa-edit"></i></a>

								<?php } ?>

								<a href="<?= $w["parseurl"] ?>" class="btn btn-color btn-xs"><?= $tl["general"]["g3"] ?></a>
							</div>
