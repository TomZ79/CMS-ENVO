...
<html>
<head>
	<title><?= $tlotv["otv_uninstall"]["otvuninst"] ?></title>
	<meta charset="utf-8">
	<!-- BEGIN Vendor CSS-->
	<?php
	...
	<div class="row">
		<div class="col-sm-12 m-t-20">
			<div class="jumbotron bg-master pt-1 pl-3 pb-1 pr-3">
				<h3 class="semi-bold text-white"><?= $tlotv["otv_uninstall"]["otvuninst"] ?></h3>
			</div>
			<hr>
			<div id="notificationcontainer"></div>
			<div class="m-b-30">

				<h4 class="semi-bold"><?= $tlotv["otv_uninstall"]["otvuninst1"] ?></h4>

				<div data-pages="card" class="card card-transparent" id="card-basic">
					<div class="card-header separator">
						<div class="card-title"><?= $tlotv["otv_uninstall"]["otvuninst2"] ?></div>
			...
			<?php if (isset($_POST['uninstall'])) {
				...
				if (isset($_POST["captcha"]) && $_POST["captcha"] != "" && $_SESSION["code"] == $_POST["captcha"]) {
					...
					if ($rows) {
						...
						$envodb->query('DROP TABLE IF EXISTS ' . DB_PREFIX . 'otv_settings_lang');
						$envodb->query('DROP TABLE IF EXISTS ' . DB_PREFIX . 'otv_settings_genre');
						$envodb->query('DROP TABLE IF EXISTS ' . DB_PREFIX . 'otv_settings_country');
						$envodb->query('DROP TABLE IF EXISTS ' . DB_PREFIX . 'otv_film');
            ...
            $(document).ready(function () {
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlotv["otv_uninstall"]["otvuninst3"]?>',
                position: 'top',
                timeout: 0,
                type: 'success'
              }).show();
              ...
              e.preventDefault();
				...
				<?php } else { ?>
					<div>
						<h5 class="text-danger bold"><?= $tlotv["otv_uninstall"]["otvuninst4"] ?></h5>
					</div>
					<script>
            $(document).ready(function () {
              'use strict';
              ...
              $('#notificationcontainer').pgNotification({
                style: 'bar',
                message: '<?=$tlotv["otv_uninstall"]["otvuninst4"]?>',
                position: 'top',
                timeout: 0,
                type: 'danger'
              }).show();
			...
			if (!$succesfully) { ?>
				<form name="company" action="uninstall.php" method="post" enctype="multipart/form-data">
					<div class="form-group form-inline">
						<label for="text"><?= $tlotv["otv_uninstall"]["otvuninst5"] ?></label>
					...
					<button type="submit" name="uninstall" class="btn btn-complete btn-block">
						<?= $tlotv["otv_uninstall"]["otvuninst6"] ?>
					</button>
				</form>
			<?php } ?>
