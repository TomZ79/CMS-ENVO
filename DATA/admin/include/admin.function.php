 ...
 *
 */
function checkstatusurl($status)
{
  $exploded_url = explode("&", $_SERVER['REQUEST_URI']);
  $exploded_url[1];

  if (in_array($status, $exploded_url)) {
    return TRUE;
  } else {
    return FALSE;
  }
    ...
    foreach ($menu['parents'][$parent] as $itemId) {
      ...
      if (!isset($menu['parents'][$itemId])) {
        ...
        $html .= '<li id="menuItem_' . $menu["items"][$itemId]["id"] . '" class="envocat">
          		...
          		<span class="actions">
          			' . ($menu["items"][$itemId]["pluginid"] == 0 && $menu["items"][$itemId]["pageid"] == 0 && $menu["items"][$itemId]["exturl"] == '' ? '<a class="btn btn-default btn-xs" href="index.php?p=page&amp;sp=newpage&amp;id=' . $menu["items"][$itemId]["id"] . '" data-toggle="tooltipEnvo" data-placement="bottom" title="' . $title1 . '"><i class="fa fa-sticky-note-o"></i></a>' : '') . '
          			' . ($menu["items"][$itemId]["pluginid"] == 0 && $menu["items"][$itemId]["pageid"] != 0 && $menu["items"][$itemId]["exturl"] == '' ? '<a class="btn btn-default btn-xs" href="index.php?p=page&amp;sp=edit&amp;id=' . $menu["items"][$itemId]["pageid"] . '" data-toggle="tooltipEnvo" data-placement="bottom" title="' . $title2 . '"><i class="fa fa-pencil"></i></a>' : '') . '
