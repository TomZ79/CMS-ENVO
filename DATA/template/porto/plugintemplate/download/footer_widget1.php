...
if (ENVO_PLUGIN_ACCESS_DOWNLOAD) {
	...
	if (isset($ENVO_DOWNLOAD_CAT) && is_array($ENVO_DOWNLOAD_CAT)) foreach ($ENVO_DOWNLOAD_CAT as $carray) {
	...
	<?php if (isset($ENVO_DOWNLOAD_CAT) && is_array($ENVO_DOWNLOAD_CAT)) foreach ($ENVO_DOWNLOAD_CAT as $c) { ?>
		...
		<li><a href="<?= $c["parseurl"] ?>">

				<?php if ($c["catimg"]) { ?>
				<imgsrc="<?= BASE_URL . $c["catimg"] ?>" alt="sideimg" />
		...
		<?php if ($catexistid) { ?>
			<ul class="nav nav-pills nav-stacked">
				<?php if (isset($ENVO_DOWNLOAD_CAT) && is_array($ENVO_DOWNLOAD_CAT)) foreach ($ENVO_DOWNLOAD_CAT as $c1) { ?><?php if ($c1["catparent"] != '0' && $c1["catparent"] == $c["id"]) { ?>
					<li><a href="<?= $c1["parseurl"] ?>"><?php if ($c1["catimg"]) { ?>
								<imgsrc="<?= BASE_URL . $c1["catimg"] ?>" alt="sideimg" /> <?php }
