...
if (isset($_POST['search'])) {
	...
	// Search query
	$result = $envodb -> query('SELECT * FROM ' . DB_PREFIX . 'download WHERE title LIKE "%' . $title . '%" LIMIT 10');
	...
	if ($row_cnt > 0) {
		...
		// Search query
		$result1 = $envodb -> query('SELECT id, title, varname FROM ' . DB_PREFIX . 'download WHERE title LIKE "%' . $title . '%" LIMIT 10');

		// Search the plugin name and id
		$result2 = $envodb -> query('SELECT id FROM ' . DB_PREFIX . 'plugins WHERE name = "Download"');
