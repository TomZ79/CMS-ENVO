	...
	<ul class="nav nav-tabs nav-tabs-responsive" role="tablist">
		...
		<li class="nav-item">
			<a href="#cmsPage4" class="" data-toggle="tab">
				<span class="text"><?= $tlporto["tplset_section_tab"]["tplsettab5"] ?></span>
			</a>
		</li>
		<li class="nav-item">
				...
				<div class="col-md-12">
					...
					<div class="box box-success">
						<div class="box-header with-border">
							...
							<?php
							// Add Html Element -> addTag (Arguments: tag, text, class, optional assoc. array)
							echo $Html -> addTag('h3', $tlporto["sp_box_title"]["spbt"], 'box-title');
							?>
						...
						<div class="box-body">
							<div class="block">
								<div class="block-content">
									<div class="row-form">
										<div class="col-md-8"><strong><?= $tlporto["sp_box_content"]["spbc"] ?></strong></div>
										<div class="col-md-4">
											...
											$data = array (
												array (
													'group'  => 'Classic',
													'option' => 'Classic - Medium',
													'value'  => 'page-header-classic,page-header-md'
												),
												array (
													'group'  => 'Classic',
													'option' => 'Classic - Large',
													'value'  => 'page-header-classic,page-header-lg'
												),
												array (
													'group'  => 'Modern',
													'option' => 'Modern - Small',
													'value'  => 'page-header-modern,page-header-sm'
												),
												array (
													'group'  => 'Modern',
													'option' => 'Modern - Medium',
													'value'  => 'page-header-modern,page-header-md'
												),
												array (
													'group'  => 'Modern',
													'option' => 'Modern - Large',
													'value'  => 'page-header-modern,page-header-lg'
												),
												array (
													'group'  => 'Background',
													'option' => 'Background - Fixed',
													'value'  => 'page-header-modern page-header-background,page-header-background-md,overlay overlay-color-dark overlay-show overlay-op-5'
												),
												array (
													'group'  => 'Background',
													'option' => 'Background - Parallax',
													'value'  => 'page-header-modern page-header-background,page-header-background-md parallax,overlay overlay-color-dark overlay-show overlay-op-5 mt-0'
												),
												array (
													'group'  => 'Background',
													'option' => 'Background - Pattern',
													'value'  => 'page-header-modern page-header-background,page-header-background-pattern,overlay overlay-color-dark overlay-show overlay-op-5'
												),
												array (
													'group'  => 'Background',
													'option' => 'Background - Overlay',
													'value'  => 'page-header-modern page-header-background,page-header-background-md,overlay overlay-color-primary overlay-show overlay-op-8'
												),
												array (
													'group'  => 'Background',
													'option' => 'Background - Clean (No Background)',
													'value'  => 'page-header-modern page-header-md,bg-transparent'
												),
												array (
													'group'  => 'Title Position',
													'option' => 'Center - Small',
													'value'  => 'page-header-modern page-header-title-position,bg-color-light-scale-1 page-header-sm',
												),
												array (
													'group'  => 'Title Position',
													'option' => 'Center - Medium',
													'value'  => 'page-header-modern page-header-title-position,bg-color-light-scale-1 page-header-md',
												),
												array (
													'group'  => 'Title Position',
													'option' => 'Center - Large',
													'value'  => 'page-header-modern page-header-title-position,bg-color-light-scale-1 page-header-lg',
												)
											);

											$priorGroup = '';
											echo '<select  name="pageheaderporto" class="form-control selectpicker">';
											foreach ($data as $entry) {
												// Start new optgroup if group changed
												if ($entry['group'] != $priorGroup) {
													if ($priorGroup != "") {
														// Close prior optgroup if prior group WAS set
														echo '</optgroup>';
													}
												...
												// Show option(s)
												if ($entry["value"] == $jktpl["pageheader_porto_tpl"]) {
													echo '<option value="' . $entry["value"] . '" selected="selected">' . $entry["option"] . '</option>';
												} else {
													echo '<option value="' . $entry["value"] . '">' . $entry["option"] . '</option>';
												}
												// Update priorGroup
												$priorGroup = $entry["group"];
											}

											// Close last optgroup + select
									...
									<div class="row-form">
										<div class="container m-0 p-0">
											<div class="row py-1">
												<div class="col-sm-12"><strong>Obrázek na pozadí</strong></div>
											</div>
											...
											foreach ($ENVO_TPL_IMG as $i) {
												...
												echo '<div class="col-md-1 text-center">#' . $i["id"] . ' <input type="hidden" name="pageheader_img_0[]" class="rowid" value="' . $i["id"] . '"/></div>';

												echo '<div class="col-md-1">Obrázek</div>';

												echo '<div class="col-md-5">';
												echo '<div class="input-group">';
												// Add Html Element -> addInput (Arguments: type, name, value, id, class, optional assoc. array)
												echo $Html -> addInput('text', 'pageheader_img_2[]', $i["img"], 'pageheader_img_' . $i["id"], 'form-control');
												echo '<span class="input-group-append">';
												// Add Html Element -> addAnchor (Arguments: href_link, text, id, class, optional assoc. array)
												echo $Html -> addAnchor('../assets/plugins/tinymce/5.3.1/plugins/filemanager/dialog.php?type=1&lang=' . $managerlangTiny . '&fldr=&field_id=pageheader_img_' . $i["id"], '<i class="pg-image"></i>', '', 'btn btn-info ifManager', array ('type' => 'button', 'data-toggle' => 'tooltipEnvo', 'data-placement' => 'bottom', 'title' => $tl["icons"]["i22"]));
												echo '</span>';
												echo '</div>';
												echo '</div>';
												...
												echo 'Stránka (ID)';
												echo $Html -> addAnchor('javascript:void(0)', '<i class="fa fa-question-circle"></i>', '', 'cms-help', array ('data-content' => $tlporto["sh_help"]["shh2"], 'data-original-title' => $tlporto["sh_help"]["shh"]));
												echo '</div>';

												echo '<div class="col-md-3">';
												echo '<div class="form-group m-0">';
												// Add Html Element -> addInput (Arguments: type, name, value, id, class, optional assoc. array)
												echo $Html -> addInput('text', 'pageheader_img_1[]', $i["catid"], '', 'form-control');
												echo '</div>';
												echo '</div>';
