...
if (ENVO_USERID) {
  ...
  if ($envouser->envoAdminAccess($envouser->getVar("usergroupid"))) {
    define('ENVO_ACCESS', TRUE);
  } else {
    define('ENVO_ACCESS', FALSE);
  }
...
if ($setting["offline"] == 1 && !ENVO_ACCESS || $USR_IP_BLOCKED) {
...
if (defined('ENVO_TEMPLATE') && defined('ENVO_PLUGIN_ID_REGISTER_FORM')) {
...
// Plugin Sitemap
if (defined('ENVO_TEMPLATE') && defined('ENVO_PLUGIN_ID_SITEMAP')) {
  if (is_numeric(ENVO_PLUGIN_ID_SITEMAP) && (ENVO_PLUGIN_ID_SITEMAP > 0)) {
    // Do something when it is defined
    $result = $envodb->query('SELECT name, varname FROM ' . DB_PREFIX . 'categories WHERE pluginid = "' . ENVO_PLUGIN_ID_SITEMAP . '" LIMIT 1');
    $PLUGIN_SM_CAT = $result->fetch_assoc();
  } else {
    // Do something else because it isn't
